...
def Init(argv=None):
	...
	parser.add_argument('-a', '--arch', nargs=1, dest='arch', default=None, help='The target architecture to build for certain platforms, available options: Windows: {}, Android: {}; WSA: {}, default to None if none given.'.format(', '.join(GetSupportedArches('Windows')), ', '.join(GetSupportedArches('Android')), ', '.join(GetSupportedArches('WSA'))))
	Undefined = 'Undefined'
