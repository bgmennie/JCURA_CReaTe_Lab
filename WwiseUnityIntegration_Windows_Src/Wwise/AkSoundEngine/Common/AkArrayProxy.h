...
{
	static  void * Alloc( size_t in_uSize )
	{
		return AkAlloc(T_MEMID, in_uSize);
	}

	static  void * ReAlloc( void * in_pCurrent, size_t in_uOldSize, size_t in_uNewSize )
	{
		return AkRealloc(T_MEMID, in_pCurrent, in_uNewSize);
	}

	static  void Free( void * in_pAddress )
	{
		AkFree(T_MEMID, in_pAddress);
	}

	static  void TransferMem(void *& io_pDest, AkArrayAllocatorNoAlign<T_MEMID> in_srcAlloc, void * in_pSrc )
	{
		io_pDest = in_pSrc;
	}
};
...
{
	template <class T>
	static  void Move(T&& io_Dest, T&& io_Src)
	{
...
// Bypass this inner struct and use a proxy in a separate header.
	...
	AKRESULT Reserve( AkUInt32 in_ulReserve )
	{
		...
		if (in_ulReserve > m_uLength)
		{
			...
			{
				if (!GrowArray(in_ulReserve - m_uLength))
					return AK_InsufficientMemory;
			}
		}
